...
float tempC = 0;
const String registarion_no = "25052";

DHT dht(DHTPIN, DHTTYPE);
WiFiClientSecure espClient; // Secure client for TLS connection

void publishData(String registration, float temperature) {
  HTTPClient https;
  ...
  if (https.begin(espClient, scriptUrl)) {
    ...
    String message = "{\"registration\":\"" + String(registarion_no) + "\",\"temperature\":\"" + String(temperature) + "\"}";
...
void loop() {
    ...
    publishData(registarion_no, tempC);
