/*
* This is a simple program to test the practical shield
* 
* Connetion help:
*  - Practical shield pins counting from left to right
*  - Practical shield data pins [0 - 7] => Arduino pins [2 - 9]
*  - Practical shield digit pins => Arduino pins [16 - 19]
*  - Practical shield gnd pins [GND_LED, GND_SSD] => Arduino pins [11 - 12]
*/
#include<Arduino.h>
...
const unsigned int SSD_DIGIT_SWITCH_PINS[4] = {16, 17, 18, 19};
const unsigned int GND_PINS[2] = {11, 12};
...
void setup() {
    ...
    for (int x = 0; x < sizeof(SSD_DIGIT_SWITCH_PINS)/sizeof(SSD_DIGIT_SWITCH_PINS[0]); x++) {
        pinMode(SSD_DIGIT_SWITCH_PINS[x], OUTPUT);
    }

    for (int x = 0; x < sizeof(GND_PINS)/sizeof(GND_PINS[0]); x++) {
        pinMode(GND_PINS[x], OUTPUT);
    }
}
...
void clearSelection() {
    for (int x = 0; x < sizeof(SSD_DIGIT_SWITCH_PINS)/sizeof(SSD_DIGIT_SWITCH_PINS[0]) ; x++) {
        digitalWrite(SSD_DIGIT_SWITCH_PINS[x], LOW);
    }
}

void LEDWrite(unsigned int idx) {
    clearData();
    digitalWrite(GND_PINS[LED_GND_INDEX], LOW);
    digitalWrite(GND_PINS[SSD_GND_INDEX], HIGH);
    digitalWrite(DATA_PINS[idx], HIGH);
}

void SSDSelect(unsigned int idx) {
    clearSelection();
    digitalWrite(SSD_DIGIT_SWITCH_PINS[idx], HIGH);
}
